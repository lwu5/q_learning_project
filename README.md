...
- Technical difficulty: To complete this project, we spent a total 26 hours. However, we would say that we spent around 5 hours just to connect to the robot. For example, we spent a decent amount of time to ensure the following: (1) check all 8 terminals are running, (2) check the robot's Lidar is spinning, (3) check the robot's battery is sufficient, (4) wait until router connection is strong, (5) remove noises in the environment, (6) rerun code until camera can is showing.
